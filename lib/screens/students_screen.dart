...
class _StudentsScreenState extends State<StudentsScreen> {
            ...
            ElevatedButton(
              onPressed: () {
                ...
                final s = Student(
                  ...
                  studentId:
                      '2024-${MockData.students.length + 1}', // Auto-generates the next ID
                  firstName: firstCtrl.text.trim(),
                  lastName: lastCtrl.text.trim(),
                  email: newEmail,
                  phone:
                      '00000000000', // Default empty phone to avoid null errors
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text('${s.firstName} added! They can now log in.'),
                  ),
                );
              },
  ...
  Widget _buildStudentList(List<Student> list) => ListView.builder(
    ...
    itemBuilder: (ctx, i) => ListTile(
      ...
      subtitle: Text(
        '${list[i].yearLevel} • ${list[i].email}',
      ), // Added email so you can see it easily
    ),
  );
}
